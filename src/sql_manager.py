...
class Instance:
    def _connect_to_db(self) -> "tuple[sqlite3.Connection, sqlite3.Cursor]":
        """
        Opens a connection and returns it, together with a cursor object.
        """
    ...
    def _save_and_close_connection(
            self,
    ...
    def __init__(self, sql_path: str):
        ...
        sqlite_con, cursor = self._connect_to_db()
...
'show_welcome_dialog')"
            ...
            pass
        self._save_and_close_connection(sqlite_con)
...
content) VALUES (?, ?, ?, ?, ?, ?)",
            ...
            for attachment in cursor.execute(
...
content) VALUES (?, ?, ?, ?, ?)",
    ...
    def import_chat(self, import_sql_path: str, chat_names: list) -> list:
        ...
        for repeated_chat in cursor.execute(
...
import.chat.id = dbchat.id"
        ...
        for repeated_message in cursor.execute(
...
dbmessage ON import.message.id = dbmessage.id"
        ...
        for repeated_attachment in cursor.execute(
